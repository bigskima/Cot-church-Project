...
export default function LeadershipHubScreen() {
  ...
  const tools = [
    ...
    { title: 'Giving Configuration', description: 'Manage church-wide giving options, purposes and bank details.', iconName: 'gift-outline', badge: 'GIVING', route: routeFor('/general/leadership/giving-manage', '/(tabs)/profile/leadership/giving-manage'), enabled: canManageGiving },
    ...
    <View style={[styles.screen, { backgroundColor: colors.bg }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.content, { paddingTop: generalWorkspace ? spacing.md : insets.top + spacing.sm, paddingBottom: generalWorkspace ? insets.bottom + spacing.xl : insets.bottom + 120 }]}>
        ...
        <View style={styles.body}>
          <SectionHeader title="Available tools" badge={authorityReady ? availableTools.length : undefined} subtitle={authorityReady ? (generalWorkspace ? 'Church ministry tools' : activeExpression?.name ? `Current space: ${activeExpression!.name}` : 'Church-level tools') : 'Checking your ministry tools'} />
          {!authorityReady ? (
