...
function NavButton({ item, onNavigate }: { item: NavItem; onNavigate?: () => void }) {
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.navItem,
        { backgroundColor: item.active ? colors.primarySoft : 'transparent', borderColor: item.active ? colors.primarySoftStrong : 'transparent' },
        pressed ? styles.pressed : null,
      ]}
    >
      <View style={[styles.navIcon, { backgroundColor: item.active ? colors.cardElevated : colors.bgSecondary }]}>
        <Icon name={item.icon as any} size={18} color={item.active ? colors.interactive : colors.textSecondary} />
      </View>
      <Text style={[styles.navLabel, { color: item.active ? colors.text : colors.textSecondary }]}>{item.label}</Text>
...
function ExpressionNavigation({ expressionId, onNavigate }: { expressionId: string; onNavigate?: () => void }) {
  ...
  const communityItems = useMemo<NavItem[]>(
    () => [
      ...
      { key: 'birthdays', label: 'Birthdays', icon: 'gift-outline', active: pathname === `${basePath}/birthdays`, onPress: () => router.push(`${basePath}/birthdays` as any) },
    ],
    [basePath, pathname],
  );
  ...
  const peopleItems = useMemo<NavItem[]>(
    () => [
      { key: 'groups', label: 'Groups', icon: 'people-circle-outline', active: pathname === `${basePath}/groups` || pathname.startsWith(`${basePath}/groups/`), onPress: () => router.push(`${basePath}/groups` as any) },
      { key: 'members', label: 'Members', icon: 'people-outline', active: pathname === `${basePath}/members`, onPress: () => router.push(`${basePath}/members` as any) },
      { key: 'leadership', label: 'Leadership', icon: 'ribbon-outline', active: pathname === `${basePath}/leadership`, onPress: () => router.push(`${basePath}/leadership` as any) },
    ],
    [basePath, pathname],
  );
  ...
  const managementItems = useMemo<NavItem[]>(
    () => {
      ...
      return [
        ...
        { key: 'manage-leadership', label: 'Manage Leadership', icon: 'people-circle-outline', active: pathname === `${manageBase}/leadership`, onPress: () => router.push(`${manageBase}/leadership` as any), enabled: management.canManageLeadership },
        { key: 'manage-invite-codes', label: 'Invite Codes', icon: 'key-outline', active: pathname === `${manageBase}/invite-codes`, onPress: () => router.push(`${manageBase}/invite-codes` as any), enabled: management.canManageInviteCodes },
        { key: 'manage-access', label: 'Team Access & Ownership', icon: 'shield-checkmark-outline', active: pathname === `${manageBase}/access`, onPress: () => router.push(`${manageBase}/access` as any), enabled: management.canManageAccess },
        { key: 'manage-settings', label: 'Expression Settings', icon: 'settings-outline', active: pathname === `${manageBase}/settings`, onPress: () => router.push(`${manageBase}/settings` as any), enabled: management.canManageSettings },
        { key: 'manage-giving', label: 'Giving Setup', icon: 'gift-outline', active: pathname === `${manageBase}/giving`, onPress: () => router.push(`${manageBase}/giving` as any), enabled: management.canManageGiving },
        { key: 'manage-finance', label: 'Giving Reports', icon: 'analytics-outline', active: pathname === `${manageBase}/finance`, onPress: () => router.push(`${manageBase}/finance` as any), enabled: management.canReadGivingFinance },
        { key: 'manage-books', label: 'Finance', icon: 'wallet-outline', active: pathname === `${manageBase}/books`, onPress: () => router.push(`${manageBase}/books` as any), enabled: management.canReadExpressionFinance },
      ]
    ...
    <View style={styles.navRoot}>
      <View style={[styles.identityCard, { backgroundColor: colors.card, borderColor: colors.borderSubtle }]}>
        <View style={[styles.identityMark, { backgroundColor: colors.primarySoft }]}>
          {expression?.avatar_url ? <Image source={{ uri: expression.avatar_url }} style={styles.identityImage} resizeMode="cover" /> : <Icon name="people" size={22} color={colors.interactive} />}
        </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.navScrollContent}>
        <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>OVERVIEW</Text>
        {overviewItems.map((item) => <NavButton key={item.key} item={item} onNavigate={onNavigate} />)}

        <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>COMMUNITY</Text>
        {communityItems.map((item) => <NavButton key={item.key} item={item} onNavigate={onNavigate} />)}

        <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>MEDIA & LIVE</Text>
        {mediaItems.map((item) => <NavButton key={item.key} item={item} onNavigate={onNavigate} />)}

        <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>GROUPS & PEOPLE</Text>
        {peopleItems.map((item) => <NavButton key={item.key} item={item} onNavigate={onNavigate} />)}

        {management.ready && managementItems.length ? (
          <>
            <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>MANAGE EXPRESSION</Text>
            {managementItems.map((item) => <NavButton key={item.key} item={item} onNavigate={onNavigate} />)}
          </>
        ) : null}
    ...
    <View style={[styles.mobileRoot, { backgroundColor: colors.bg }]}>
      ...
      <Modal visible={drawerOpen} transparent animationType="fade" onRequestClose={() => setDrawerOpen(false)}>
        <View style={styles.modalRoot}>
          <Pressable style={styles.backdrop} onPress={() => setDrawerOpen(false)} accessibilityRole="button" accessibilityLabel="Close Expression navigation" />
          <View style={[styles.drawer, { paddingTop: Math.max(insets.top + spacing.sm, spacing.lg), paddingBottom: Math.max(insets.bottom, spacing.md), backgroundColor: colors.bgSecondary, borderColor: colors.borderSubtle }, shadows.floating]}>
            <View style={styles.drawerHeader}>
              <Text style={[styles.drawerEyebrow, { color: colors.textMuted }]}>COT EXPRESSION</Text>
              <Pressable onPress={() => setDrawerOpen(false)} accessibilityRole="button" accessibilityLabel="Close Expression navigation" style={({ pressed }) => [styles.closeButton, { backgroundColor: colors.bg, borderColor: colors.borderSubtle }, pressed ? styles.pressed : null]}>
                <Icon name="close" size={20} color={colors.text} />
              </Pressable>
            </View>
            <ExpressionNavigation expressionId={expressionId} onNavigate={() => setDrawerOpen(false)} />
          </View>
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  navIcon: { width: 32, height: 32, borderRadius: 11, alignItems: 'center', justifyContent: 'center' },
  navLabel: { flex: 1, fontSize: 13, lineHeight: 18, fontWeight: '700' },
  pressed: { opacity: 0.82, transform: [{ scale: 0.985 }] },
  modalRoot: { flex: 1, flexDirection: 'row' },
  backdrop: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, backgroundColor: 'rgba(0,0,0,0.48)' },
  drawer: { width: '84%', maxWidth: 360, height: '100%', borderRightWidth: 1, paddingHorizontal: spacing.md },
  drawerHeader: { minHeight: 46, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: spacing.sm },
  drawerEyebrow: { fontSize: 10, fontWeight: '900', letterSpacing: 1 },
  closeButton: { width: 36, height: 36, borderRadius: radius.pill, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
});
