...
export function ReelPlayer({
  ...
  const handleShare = async () => {
    ...
    try { await Share.share({ message: `Watch this Reel on City of Transformation: ${reel.caption || 'Church video'}` }); } catch {}
  };
    ...
    <View style={[styles.container, containerHeight ? { height: containerHeight } : null]}>
      <View style={styles.videoCanvas}>
        {videoUrl && player ? (
          ...
          <View style={styles.placeholderCanvas}><Icon name="film-outline" size={48} color="#1D9BF0" /></View>
        )}

        {isBuffering ? <View style={styles.bufferingOverlay}><ActivityIndicator size="large" color="#FFFFFF" /></View> : null}
        <Pressable onPress={togglePlay} style={styles.tapLayer}>
          {!isPlaying ? <View style={styles.pauseIndicator}><Icon name="play" size={36} color="#FFFFFF" style={{ marginLeft: 3 }} /></View> : null}
        </Pressable>
        ...
        <View style={styles.actionRail}>
          ...
          <Pressable onPress={openInlineComments} hitSlop={6} style={styles.actionBtn} accessibilityRole="button" accessibilityLabel="Comments">
            <View style={styles.actionCircle}><Icon name="chatbubble-ellipses-outline" size={24} color="#FFFFFF" /></View>
            <Text style={styles.actionLabel}>{reel.comments_count}</Text>
          </Pressable>

          <Pressable onPress={toggleSave} hitSlop={6} style={styles.actionBtn} accessibilityRole="button" accessibilityLabel="Bookmark reel">
            <View style={[styles.actionCircle, isSaved && styles.savedCircle]}><Icon name={isSaved ? 'bookmark' : 'bookmark-outline'} size={24} color={isSaved ? '#168FF0' : '#FFFFFF'} /></View>
            <Text style={styles.actionLabel}>Save</Text>
          </Pressable>

          <Pressable onPress={handleShare} hitSlop={6} style={styles.actionBtn} accessibilityRole="button" accessibilityLabel="Share reel">
            <View style={styles.actionCircle}><Icon name="share-social-outline" size={24} color="#FFFFFF" /></View>
            <Text style={styles.actionLabel}>Share</Text>
          </Pressable>

          {onReport ? (
            <Pressable onPress={onReport} hitSlop={6} style={styles.actionBtn} accessibilityRole="button" accessibilityLabel="Report reel">
              <View style={styles.actionCircle}><Icon name="flag-outline" size={22} color="#FFFFFF" /></View>
...
const styles = StyleSheet.create({
  container: { width: '100%', height: '100%', backgroundColor: '#000000' },
  videoCanvas: { width: '100%', height: '100%', position: 'relative', overflow: 'hidden' },
  videoView: { width: '100%', height: '100%' },
  videoPoster: { width: '100%', height: '100%' },
  placeholderCanvas: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#050B14' },
  bufferingOverlay: { ...StyleSheet.absoluteFill as any, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0, 0, 0, 0.3)' },
  tapLayer: { ...StyleSheet.absoluteFill as any, alignItems: 'center', justifyContent: 'center' },
  pauseIndicator: { width: 68, height: 68, borderRadius: 34, backgroundColor: 'rgba(0, 0, 0, 0.65)', alignItems: 'center', justifyContent: 'center' },
  actionRail: { position: 'absolute', right: spacing.md, bottom: 90, alignItems: 'center', gap: spacing.lg, zIndex: 10 },
  actionBtn: { alignItems: 'center', gap: 4 },
  actionCircle: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(0, 0, 0, 0.45)', alignItems: 'center', justifyContent: 'center' },
  likedCircle: { backgroundColor: 'rgba(239, 68, 68, 0.25)' },
  savedCircle: { backgroundColor: 'rgba(22, 143, 240, 0.25)' },
  ...
  followBtn: { paddingHorizontal: 12, paddingVertical: 4, borderRadius: radius.pill, backgroundColor: 'rgba(255, 255, 255, 0.2)', borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.4)' },
  followingBtn: { backgroundColor: 'transparent', borderColor: 'rgba(255, 255, 255, 0.6)' },
