      ...
      {resource.loading && !resource.data ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            if (item.kind === 'reel') return <View style={[styles.timelineItem, { width: Math.min(contentWidth, 920) }]}><ReelCard reel={item.reel} width={reelWidth} variant="feed" commentContext="public" onPress={() => router.push({ pathname: '/general/reels', params: { reelId: item.reel.id } } as any)} onOpenComments={item.reel.content_items?.id ? () => router.push({ pathname: '/general/comments/[contentId]', params: { contentId: item.reel.content_items!.id } } as any) : undefined} /></View>;
