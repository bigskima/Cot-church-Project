...
export function PrayerExperience({ scope = 'general', embedded = false }: { scope?: PrayerScope; embedded?: boolean }) {
  ...
  const [wallScope, setWallScope] = useState<PrayerScope>(scope);
  const [destination, setDestination] = useState<PrayerScope>(scope);
  ...
  useEffect(() => {
    if (wallScope !== scope) setWallScope(scope);
    if (destination !== scope) setDestination(scope);
  }, [destination, scope, wallScope]);
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.bg }]}>
      ...
      >
        {embedded ? (
          <View style={[styles.embeddedHeader, { backgroundColor: colors.card, borderColor: colors.borderSubtle }]}>
            <View style={styles.flex}>
              <Text style={[styles.embeddedEyebrow, { color: colors.interactive }]}>EXPRESSION PRAYER</Text>
              <Text style={[styles.embeddedTitle, { color: colors.text }]}>Prayer</Text>
              <Text style={[styles.embeddedCopy, { color: colors.textSecondary }]}>
                Private prayer ministry for {expression?.name ?? 'this Expression'}.
              </Text>
            </View>
            <Button label="Send prayer" onPress={openSubmission} size="sm" />
          </View>
        ) : (
        ...
        {submittedSuccess ? (
          ...
          <View style={styles.form}>
            ...
            <View style={styles.optionStack}>
              ...
              {scope === 'expression' && canUseExpressionScope && expression ? (
                <Pressable
...
export default function GeneralPrayerExperience() {
