...
export function LivePlayerExperience({ streamId: id, scope = 'general', embedded = false }: { streamId: string; scope?: 'general' | 'expression'; embedded?: boolean }) {
  ...
  const loadChat = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      if (showLoading) setInteractionError(value instanceof Error ? value.message : 'Unable to load live chat.');
    } finally {
  ...
  useEffect(() => {
    ...
    void loadChat(true);
    if (!isLive) return;
    const interval = setInterval(() => { void loadChat(false); }, 5000);
  ...
  useEffect(() => {
    ...
    const updatePresence = (action: 'heartbeat' | 'leave') => api.request('stream-presence', {
      method: 'POST', context: requestContext, body: JSON.stringify({ sessionId: viewerSessionId, action }),
    });
    const interval = setInterval(() => { void updatePresence('heartbeat').catch(() => {}); }, 30_000);
    return () => {
  ...
  const submitSupport = async () => {
    ...
    try {
      await api.request('live-interactions', {
        method: 'POST', context: requestContext,
        body: JSON.stringify({ action: 'follow_up', streamId: id, type: supportType }),
      });
      setSupportSent(true);
    } catch (value) {
  ...
  if (loading) {
    return <View style={[styles.screen, { backgroundColor: colors.bg, paddingTop: topInset }]}><Skeleton height={240} /><View style={styles.loadingBody}><Skeleton height={28} width="60%" /><Skeleton height={20} width="40%" /><Skeleton height={120} /></View></View>;
  }

  if (error || !access) {
    return <View style={[styles.screen, { backgroundColor: colors.bg, paddingTop: topInset }]}><Pressable onPress={() => router.back()} style={styles.backBtn}><Icon name="arrow-back" size={20} color={colors.text} /><Text style={[styles.backText, { color: colors.text }]}>Back to Live</Text></Pressable><View style={styles.errorPad}><ResourceError message={error || 'Broadcast unavailable.'} /></View></View>;
  }
    ...
    >
      ...
      <View style={styles.chatSection}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <View style={styles.chatBubble}><Avatar url={item.avatarUrl} name={item.user} size="xs" /><View style={styles.chatCopy}><View style={styles.chatMetaRow}><Text style={[styles.chatUser, { color: colors.text }]}>{item.user}</Text><Text style={[styles.chatTime, { color: colors.textMuted }]}>{new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text></View><Text style={[styles.chatText, { color: colors.textSecondary }]}>{item.text}</Text></View></View>}
          ListEmptyComponent={() => <View style={styles.chatEmpty}><Icon name="chatbubbles-outline" size={24} color={colors.textMuted} /><Text style={[styles.chatEmptyText, { color: colors.textMuted }]}>{mode === 'visitor' ? isLive ? 'Sign in to see and participate in live fellowship chat.' : 'Sign in when you want to join live fellowship during a broadcast.' : !showFellowshipHistory ? 'Live fellowship opens when this broadcast begins.' : isLive ? 'No chat messages yet.' : 'No fellowship messages were saved for this broadcast.'}</Text></View>}
        />
        ...
        {access.canChat ? (
          <View style={[styles.chatInputBar, { backgroundColor: colors.glass, borderColor: colors.borderSubtle, marginBottom: Math.max(insets.bottom, spacing.sm) }, shadows.floating]}>
            <TextInput value={chatMessage} onChangeText={setChatMessage} placeholder="Message live fellowship…" placeholderTextColor={colors.textMuted} multiline style={[styles.chatInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.borderSubtle }]} />
            <Pressable onPress={() => void handleSendChat()} disabled={!chatMessage.trim()} style={({ pressed }) => [styles.sendChatBtn, { backgroundColor: chatMessage.trim() ? colors.interactive : colors.bgSecondary, opacity: pressed ? 0.8 : 1 }]}><Icon name="arrow-up" size={18} color={chatMessage.trim() ? '#FFFFFF' : colors.textMuted} /></Pressable>
          </View>
        ) : mode === 'visitor' ? (
          <Pressable onPress={() => router.push({ pathname: '/(auth)/login', params: { returnTo } } as any)} accessibilityRole="button" style={[styles.signInChat, { backgroundColor: colors.glass, borderColor: colors.borderSubtle, marginBottom: Math.max(insets.bottom, spacing.sm) }, shadows.floating]}><Icon name="log-in-outline" size={17} color={colors.interactive} /><Text style={[styles.signInChatText, { color: colors.interactive }]}>Sign in for live fellowship</Text></Pressable>
        ) : (
...
const styles = StyleSheet.create({
  ...
  chatHeading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: spacing.md, paddingVertical: spacing.xs },
  chatHeadingText: { fontSize: 14, fontWeight: '800' },
  chatStatus: { fontSize: 11 },
  interactionErrorCard: { flexDirection: 'row', alignItems: 'center', gap: 7, marginHorizontal: spacing.md, paddingHorizontal: spacing.sm, paddingVertical: 7, borderRadius: radius.lg },
  ...
  chatBubble: { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.sm, paddingVertical: 7 },
  ...
  chatInputBar: { flexDirection: 'row', alignItems: 'flex-end', marginHorizontal: spacing.md, padding: spacing.sm, borderWidth: 1, borderRadius: radius.xxl, gap: spacing.sm },
  chatInput: { flex: 1, borderRadius: radius.lg, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 9, maxHeight: 96, fontSize: 13 },
  sendChatBtn: { width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center' },
  ...
  supportOption: { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.sm, padding: spacing.md, borderWidth: 1, borderRadius: radius.xl },
