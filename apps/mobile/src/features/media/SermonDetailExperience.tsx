...
export function SermonDetailExperience({ sermonId: id, scope = 'general' }: { sermonId: string; scope?: 'general' | 'expression' }) {
  ...
  const resource = useResource<Sermon>(
    `sermon:detail:${
      expressionMode
        ? `expression:${activeExpressionId ?? 'none'}`
        : 'public'
    }:${id}`,
    async (signal) => {
      if (!expressionMode) {
        if (!activeOrganizationId) {
          return Promise.reject(
            new Error('Choose a church to view this sermon.'),
          );
        }

        return api.request<Sermon>(
          `public-content?type=sermon&id=${encodeURIComponent(
            id,
      ...
      if (!activeExpressionId) {
        return Promise.reject(
          new Error('Enter this Expression to view its internal sermon.'),
        );
      }
  ...
  const playback = useResource<SermonPlayback>(
    ...
    (signal) => {
      if (!expressionMode) {
        if (!activeOrganizationId) {
          return Promise.reject(
            new Error('Choose a church to play this sermon.'),
          );
        }

        return api.request<SermonPlayback>(
          `sermon-playback?id=${id}&organizationId=${encodeURIComponent(
      ...
      if (!activeExpressionId) {
        return Promise.reject(
          new Error('Enter this Expression to play its internal sermon.'),
        );
      }

      return api.request<SermonPlayback>(
        `sermon-playback?id=${id}`,
        { signal },
      );
    },
  );
  ...
  const engagement = useResource<{
    ...
    (signal) =>
      mode === 'authenticated' && contentId
        ? api.request(
            `engagement?contentId=${contentId}&view=state`,
            {
              signal,
  ...
  const directVideoUrl =
    sermon?.video_url ||
    sermon?.media_assets?.renditions?.find(
      (rendition) => rendition.rendition_kind === 'video_stream',
    )?.storage_path ||
    (sermon?.media_assets?.media_type === 'video'
  ...
  const directAudioUrl =
    sermon?.audio_url ||
    sermon?.media_assets?.renditions?.find(
      (rendition) => rendition.rendition_kind === 'audio_stream',
    )?.storage_path ||
    (sermon?.media_assets?.media_type === 'audio'
      ? sermon.media_assets?.url
      : undefined);
  ...
  const posterUrl =
    playback.data?.posterUrl ||
    sermon?.thumbnail_url ||
    sermon?.media_assets?.thumbnailUrl ||
    sermon?.media_assets?.url;

  const durationSeconds =
    playback.data?.durationSeconds ||
    sermon?.duration_seconds ||
    sermon?.media_assets?.duration_seconds;
  ...
  const syncProgress = useCallback(
    (seconds: number, duration: number) => {
      ...
      if (
        wholeSecond - lastSyncedSecond.current < 15 &&
          ...
          <View style={styles.body}>
            {posterUrl ? (
              <Pressable
                ...
                accessibilityLabel={`View full ${sermon.title} banner`}
              >
                <Image
            ...
            {playback.loading && !hasVideo && !hasAudio ? (
              ...
              >
                <Text
                  style={[
                    styles.mediaStateTitle,
                    { color: colors.text },
                  ]}
                >
                  Preparing sermon media…
                </Text>
              </View>
            ) : mediaPending ? (
              <View
              ...
              >
                ...
                <Text
                  style={[
                    styles.mediaStateText,
                    { color: colors.textSecondary },
                  ]}
                >
                  Playback will become available automatically when the
                  recording is ready.
                </Text>
              </View>
              ...
              <VideoPlayer
                ...
                initialPositionSeconds={
                  engagement.data?.progress?.completed
                    ? 0
                    : engagement.data?.progress?.progress_seconds ?? 0
                }
              ...
              >
                ...
                >
                  This sermon can still be read below while its media is
                  being prepared.
                </Text>
              </View>
            )}
            ...
            {sermon.scripture_references?.length ? (
              ...
              >
                ...
                <View style={styles.scripturePills}>
                  {sermon.scripture_references.map(
                    (reference, index) => (
                      <View
                        key={`${reference}-${index}`}
                        style={[
        ...
        media={
          posterUrl
            ? {
                url: posterUrl,
                type: 'image',
                title: `${sermon?.title || 'Sermon'} banner`,
              }
            : null
        }
...
export default function GeneralSermonDetailExperience({ sermonId }: { sermonId: string }) {
