...
import { PLATFORM_MODULES } from './PlatformAdministrationHub';
import { PlatformGovernanceControls } from './PlatformGovernanceControls';
import { PlatformAccessControls } from './PlatformAccessControls';
import { PlatformServiceControls } from './PlatformServiceControls';
...
export default function PlatformAdministrationModuleExperience() {
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.bg }]}>
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={[styles.content, { paddingTop: insets.top + spacing.sm, paddingBottom: insets.bottom + 96 }]}>
        ...
        {authority.loading ? <Skeleton height={110} count={3} /> : !authority.data || !livePermission || !allowed ? <EmptyState title="This administration section is unavailable" message="Your current Platform Administration role does not grant this section." iconName="lock-closed-outline" /> : (
          <>
            ...
            {data.loading && data.data === undefined ? <Skeleton height={104} count={4} /> : data.error && data.data === undefined ? <ResourceError message={data.error} retry={data.refresh} /> : (
              <>
                <PlatformGovernanceControls moduleKey={module.key} data={data.data} refresh={data.refresh} authority={authority.data} />
                <PlatformAccessControls moduleKey={module.key} data={data.data} refresh={data.refresh} authority={authority.data} />
                <PlatformServiceControls moduleKey={module.key} data={data.data} refresh={data.refresh} authority={authority.data} />

                <View style={styles.sectionHeading}><View style={styles.flex}><Text style={[styles.sectionTitle, { color: colors.text }]}>Operational snapshot</Text><Text style={[styles.sectionCopy, { color: colors.textMuted }]}>Live backend response for verification and operational context.</Text></View><Button label="Refresh" variant="outline" size="sm" onPress={data.refresh} loading={data.refreshing} /></View>
                <DataPreview value={data.data} />
              </>
            )}
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  sectionHeading: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, marginTop: spacing.md }, sectionTitle: { fontSize: 18, lineHeight: 23, fontWeight: '900' }, sectionCopy: { fontSize: 10.5, lineHeight: 15, marginTop: 2 }, list: { gap: spacing.sm }, group: { gap: spacing.sm }, groupHeading: { flexDirection: 'row', alignItems: 'center', gap: 7 }, groupTitle: { fontSize: 14, lineHeight: 19, fontWeight: '900' },
