...
export function AiInfrastructure({api}:{api:ApiClient}){
  ...
  return <div>
    <div className="admin-stats-grid">
      <StatWidget title="AI Providers" value={`${data.providers.filter(provider=>provider.status==='active').length} active`} subtitle={`${data.providers.length} registered adapters`} trend={{value:data.providers.some(provider=>provider.status==='degraded')?'DEGRADED PROVIDER':'ROUTING READY',isPositive:!data.providers.some(provider=>provider.status==='degraded')}} icon="✦" variant="gold" />
      <StatWidget title="Active Models" value={activeModels.length} subtitle={`${data.globalRoutes.filter(route=>route.is_active).length} global capability routes`} trend={{value:'DATABASE DRIVEN',isPositive:true}} icon="🧠" />
      <StatWidget title="Recent AI Safety" value={`${awaitingReview} review`} subtitle={`${recentFailures} failures in the recent run window`} trend={{value:recentFailures?'REVIEW FAILURES':'NO RECENT FAILURES',isPositive:recentFailures===0}} icon="🛡" variant="success" />
    </div>
    ...
    <Card title="AI Provider Adapters" subtitle="Paste API keys securely when configuring a provider. Raw values are encrypted in Platform Vault; routing stores only the stable secret reference." headerAction={<Button variant="outline" size="sm" onClick={()=>void load()} loading={loading}>Refresh</Button>}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(300px, 1fr))',gap:16}}>{data.providers.map(provider=><div key={provider.id} style={{backgroundColor:'var(--bg-elevated)',border:'1px solid var(--border-subtle)',borderRadius:'var(--radius-lg)',padding:20}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:14}}>{(modelsByProvider.get(provider.id)??[]).map(model=><span key={model.id} style={{fontSize:10,border:'1px solid var(--border-subtle)',borderRadius:6,padding:'3px 7px',color:model.is_active?'var(--text-primary)':'var(--text-muted)'}}>{model.model_key}</span>)}</div>
    ...
    <Card title="Intelligence Capabilities & Human-Review Matrix" subtitle="Capabilities and pastoral-risk policy are stored in the backend; routes are configured globally here.">
    ...
    <Modal isOpen={!!providerModal} onClose={()=>{if(!busy)closeProvider();}} title={providerModal?`Configure ${providerModal.name}`:'Configure AI provider'} subtitle="Paste the API key here if you are adding or rotating it. It is encrypted in Supabase Vault and cannot be read back from this Admin UI. Leave the API-key field blank to reuse the existing Vault/environment credential." footer={<div style={{display:'flex',gap:12}}><Button variant="outline" disabled={busy} onClick={closeProvider}>Cancel</Button><Button variant="gold" loading={busy} onClick={()=>void saveProvider()}>Encrypt key & save provider</Button></div>}>
      ...
      <label style={{display:'grid',gap:6,margin:'14px 0'}}><strong style={{fontSize:12}}>Provider status</strong><select className="search" value={providerStatus} onChange={event=>setProviderStatus(event.target.value as AiProvider['status'])}><option value="active">Active</option><option value="degraded">Degraded</option><option value="disabled">Disabled</option></select></label>
    ...
    <Modal isOpen={!!routeCapability} onClose={()=>{if(!busy)setRouteCapability(null);}} title={routeCapability?`Route ${routeCapability.name}`:'Configure AI route'} subtitle="Select an active primary model. Fallback IDs already stored for this route are preserved." footer={<div style={{display:'flex',gap:12}}><Button variant="outline" disabled={busy} onClick={()=>setRouteCapability(null)}>Cancel</Button><Button variant="gold" loading={busy} onClick={()=>void saveRoute()}>Save route</Button></div>}>
      <label style={{display:'grid',gap:6,marginBottom:14}}><strong style={{fontSize:12}}>Primary model</strong><select className="search" value={routePrimaryModel} onChange={event=>setRoutePrimaryModel(event.target.value)}><option value="">Select a model</option>{activeModels.map(model=><option key={model.id} value={model.id}>{model.display_name} · {model.ai_providers?.code}</option>)}</select></label>
    ...
    <Modal isOpen={modelModalOpen} onClose={()=>{if(!busy)setModelModalOpen(false);}} title="Register AI model" subtitle="Model identifiers and pricing are configuration, not hardcoded application logic." footer={<div style={{display:'flex',gap:12}}><Button variant="outline" disabled={busy} onClick={()=>setModelModalOpen(false)}>Cancel</Button><Button variant="gold" loading={busy} onClick={()=>void saveModel()}>Save model</Button></div>}>
      <label style={{display:'grid',gap:6,marginBottom:14}}><strong style={{fontSize:12}}>Provider</strong><select className="search" value={modelProviderId} onChange={event=>setModelProviderId(event.target.value)}><option value="">Select provider</option>{data.providers.map(provider=><option key={provider.id} value={provider.id}>{provider.name}</option>)}</select></label>
