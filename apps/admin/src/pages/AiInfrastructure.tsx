...
export function AiInfrastructure({api,canManage=false,canManageSecrets=false}:{api:ApiClient;canManage?:boolean;canManageSecrets?:boolean}){
  ...
  return <div className="admin-page-stack">
    ...
    <Card title="Model registry" subtitle="Models, context windows and cost metadata used by the routing engine" headerAction={canManage?<Button variant="gold" size="sm" onClick={()=>{setModelProviderId(data.providers[0]?.id??'');setModelModalOpen(true);}}>Add model</Button>:undefined}>
