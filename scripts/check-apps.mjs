...
const checks = [
  ...
  [/Share with General COT[\s\S]*Voice[\s\S]*\/general\/studio\/reel[\s\S]*\/general\/studio\/video/, 'General Home exposes the four public creation formats in the compact composer'],
  [/Create something[\s\S]*no ministry role is required[\s\S]*More tools/, 'General Create separates public creation from privileged ministry tools'],
  ...
  [/mode === 'authenticated' \? \([\s\S]*accessibilityLabel="Create"[\s\S]*\/general\/studio/, 'General Home exposes Create to every authenticated user'],
