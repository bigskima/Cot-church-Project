...
const checks = [
  ...
  [/microphonePermission[\s\S]*recordAudioAndroid[\s\S]*enableBackgroundRecording[\s\S]*false/, 'microphone recording is configured without background capture'],
  [/useAudioRecorder[\s\S]*requestRecordingPermissionsAsync[\s\S]*Stop & attach/, 'General composer records and attaches microphone audio'],
