...
const files = [
  ...
  'apps/mobile/app/general/tools.tsx',
  ...
  'apps/mobile/src/components/profile/SocialProfileHero.tsx',
  'apps/mobile/src/components/media/AdaptiveMediaImage.tsx',
  'apps/mobile/src/components/media/MediaPreviewModal.tsx',
  'apps/mobile/src/components/identity/PublicIdentityBadge.tsx',
  'apps/mobile/src/features/general/GeneralHomeExperience.tsx',
  'apps/mobile/src/features/general/GeneralHomeActionDeck.tsx',
  'apps/mobile/src/features/chat/ExpressionChatExperience.tsx',
  'apps/mobile/src/features/chat/RichMessageBubble.tsx',
  ...
  'apps/mobile/src/components/engagement/ContentReportSheet.tsx',
  ...
  'apps/mobile/src/components/media/ReelPlayer.tsx',
  ...
  'apps/mobile/src/features/expression/GroupSpaceExperience.tsx',
  ...
  'apps/admin/src/pages/UserGovernance.tsx',
  ...
  'apps/admin/src/components/AdminGuide.tsx',
...
const checks = [
  ...
  [/Temporary chat[\s\S]*clears from view when you refresh this screen or close COT/, 'COT AI clearly labels conversations as temporary session-only chat'],
  [/Private credentials stay protected[\s\S]*urgent safety messages may alert authorised pastoral care/, 'COT AI keeps a compact privacy and urgent-safety disclosure'],
  [/cycleSpeechRate[\s\S]*Voice \{speechRate\}×/, 'COT AI exposes compact cycling voice-speed control'],
  ...
  [/ContentReportSheet[\s\S]*contentId: post\.id[\s\S]*isExpressionPost \? 'current' : 'public'/, 'shared posts report through exact General or Expression context'],
  [/flag-outline[\s\S]*Report[\s\S]*ContentReportSheet[\s\S]*commentId: reportTarget\.id[\s\S]*context: reportContext/, 'shared comment threads expose scoped reporting'],
  [/onReport=\{\(\) => handleReportReel\(item\)\}[\s\S]*onReport\?: \(\) => void[\s\S]*accessibilityLabel="Report reel"/, 'Reels expose member reporting in the immersive action rail'],
  [/handleReportReel[\s\S]*content_items\?\.id[\s\S]*ContentReportSheet[\s\S]*expressionId \? 'current' : 'public'/, 'Reel reports preserve exact General or Expression context'],
  [/handleReport[\s\S]*This video is not ready to report yet[\s\S]*flag-outline[\s\S]*Report[\s\S]*ContentReportSheet/, 'Watch detail exposes authenticated member reporting'],
  [/Report this sermon if something needs moderation review[\s\S]*ContentReportSheet[\s\S]*expressionMode \? 'current' : 'public'/, 'sermon detail exposes scoped member reporting'],
  ...
  [/key: 'moderation'[\s\S]*label: 'Moderation'[\s\S]*platform\.moderation\.read/, 'Platform Administration exposes permission-aware moderation navigation'],
  [/case 'moderation'[\s\S]*ModerationCenter[\s\S]*platform\.moderation\.manage/, 'Moderation workspace uses dedicated manage capability'],
  [/title: 'Open'[\s\S]*title: 'Paused'[\s\S]*title: 'Approved accounts only'[\s\S]*title="Public posting"/, 'Moderation workspace exposes three clear General posting modes'],
  [/add_public_posting_exemption[\s\S]*Approved public posters[\s\S]*Find an account to approve/, 'Moderation workspace manages explicit public posting approvals'],
  [/individual posting restriction always blocks[\s\S]*Private Expression posting is not controlled/, 'Admin UI explains individual restriction precedence and Expression boundary'],
  [/restore_posting[\s\S]*restrict_posting/, 'Account governance retains individual posting suspension controls'],
  [/Public Posting[\s\S]*Reports[\s\S]*Content[\s\S]*Deletion History/, 'Moderation workspace is layered into posting, reports, content, and deletion history'],
  [/resolve_report[\s\S]*Review[\s\S]*Hide[\s\S]*Dismiss[\s\S]*Delete/, 'Moderation reports expose review, hide, dismiss, and delete actions'],
  [/view: 'content'[\s\S]*General COT only[\s\S]*Expressions only[\s\S]*All supported content/, 'Platform content moderation separates General and Expression filters'],
  [/action: 'remove_content'[\s\S]*Permanently remove/, 'Platform content deletion uses an explicit destructive confirmation flow'],
  [/action: 'remove_comment'[\s\S]*Comment and its replies were removed/, 'Platform comment deletion communicates recursive thread removal'],
  [/retry_storage_cleanup[\s\S]*title="Deletion history"[\s\S]*Retry cleanup/, 'Deletion history exposes retryable Storage cleanup'],
  [/deleted_at[\s\S]*label="DELETED"[\s\S]*Delete Expression/, 'Expression governance displays irreversible deleted state and protected delete action'],
  [/method: 'DELETE'[\s\S]*confirmation: deleteConfirmation\.trim\(\)[\s\S]*label=\{\`Type \$\{deleteExp\.code\} to confirm\`\}/, 'Expression deletion requires exact code confirmation'],
  [/Historical financial and audit records were preserved[\s\S]*active invite codes are revoked/, 'Expression deletion UI communicates preserved evidence and invite revocation'],
  [/item\.deleted_at \? \([\s\S]*DELETED[\s\S]*!item\.deleted_at/, 'Deleted Expressions do not expose restore or delete lifecycle controls'],
  ...
  [/const canPublishPublic = generalWorkspace && mode === 'authenticated'/, 'General Reel and Watch creation are available to signed-in accounts'],
  [/const canPublishExpression =[\s\S]*Boolean\(expression\?\.id\)[\s\S]*hasCapability\('reels\.publish'\)/, 'Expression Reel publishing requires active Expression authority'],
  [/const canPublishExpression =[\s\S]*Boolean\(expression\?\.id\)[\s\S]*hasCapability\('videos\.publish'\)/, 'Expression Watch publishing requires active Expression authority'],
  [/canPostGeneral = mode === 'authenticated'[\s\S]*canPostExpression/, 'General posting is authenticated while Expression posting stays a separate lane'],
  [/canAttachAudio = canPostDestination/, 'General post composer permits authenticated audio attachments'],
  [/canCreatePublicReel = mode === 'authenticated'/, 'General community exposes Reel creation to signed-in accounts'],
  [/canCreatePublicVideo = mode === 'authenticated'/, 'General community exposes long-form video creation to signed-in accounts'],
  [/expo-audio[\s\S]*~57\.0\.4/, 'Expo audio recording dependency is pinned for the active SDK'],
  [/microphonePermission[\s\S]*recordAudioAndroid[\s\S]*enableBackgroundRecording[\s\S]*false[\s\S]*enableBackgroundPlayback[\s\S]*false/, 'microphone recording is configured without background capture or playback services'],
  [/useAudioRecorder[\s\S]*requestRecordingPermissionsAsync[\s\S]*audio\/webm[\s\S]*Stop & attach/, 'General composer records and attaches native and browser microphone audio'],
  [/Explore COT[\s\S]*priorityItems\.slice\(0, 3\)[\s\S]*View more/, 'General Home limits quick routes to three essentials plus View more'],
  [/accessibilityLabel="Create"[\s\S]*openGeneralComposer\('post'\)/, 'General Home keeps authenticated publishing behind the floating Create action'],
  ...
  [/feedIdentityBlock[\s\S]*identityHeader[\s\S]*contextRow[\s\S]*feedBodyBlock/, 'feed post identity stays visually integrated inside the shared post card'],
  ...
  [/GroupSpaceExperience groupId=\{id\}/, 'Expression group detail opens the scoped Group space experience'],
  ...
  [/ExpressionManagementHub[\s\S]*CompactRouteGrid/, 'Expression tools use the compact route workspace'],
  ...
  [/ExpressionManagementHeader[\s\S]*CompactRouteGrid/, 'Expression operations use compact reusable navigation'],
  [/BottomSheet[\s\S]*compact[\s\S]*CompactRouteGrid/, 'Expression quick-route sheets use compact BottomSheet chrome'],
  ...
  [/ExpressionContentStudio[\s\S]*CompactRouteGrid/, 'Expression Content Studio uses compact reusable creation navigation'],
  ...
  [/name="studio\/reel" options=\{hidden as any\}/, 'General tab shell hides nested studio routes'],
  [/name="leadership\/media-studio" options=\{hidden as any\}/, 'General tab shell hides nested leadership routes'],
  [/name="comments\/\[contentId\]" options=\{hidden as any\}/, 'General tab shell hides nested content routes'],
  ...
  [/\/general\/tools[\s\S]*General COT tools and settings/, 'General Home routes secondary utilities into the simplified tools hub'],
  [/GeneralToolsScreen[\s\S]*CompactRouteGrid/, 'General Tools uses reusable compact route grids'],
  ...
  [/key: 'live'[\s\S]*key: 'sermons'[\s\S]*key: 'videos'[\s\S]*key: 'reels'[\s\S]*CompactRouteGrid/, 'Expression media header keeps media destinations connected in a compact route grid'],
  ...
  [/canPostGeneral = mode === 'authenticated'/, 'signed-in General Community publishing'],
  ...
  [/Long-form media[\s\S]*Quick discovery[\s\S]*Public conversations[\s\S]*Church & leaders/, 'General Explore exposes a compact discovery deck'],
  [/compactAll = activeFilter === 'all' && !query\.trim\(\)[\s\S]*slice\(0, 3\)/, 'General Explore reduces long catalogue scrolling only outside active search'],
  [/Latest sermons[\s\S]*Featured series[\s\S]*Upcoming gatherings/, 'General Explore presents focused discovery sections'],
  [/Public Expression profile[\s\S]*Members, groups, internal announcements and management tools remain private/, 'public Expression profile states the private workspace boundary'],
  [/label="Overview"[\s\S]*NEXT GATHERING[\s\S]*LATEST MESSAGE[\s\S]*LATEST VIDEO/, 'public Expression profile opens with a layered overview'],
  [/returnTo: `\/general\/expression\/\$\{id\}`/, 'public Expression sign-in returns to canonical General profile route'],
