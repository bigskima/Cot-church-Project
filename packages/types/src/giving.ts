...
export interface Donation{id:string;branch_id:string|null;group_id:string|null;campaign_id:string|null;purpose_id?:string|null;amount_minor:number;currency:string;status:DonationStatus;anonymous:boolean;succeeded_at:string|null;created_at:string;}
export interface DonationIntentRequest{branchId?:string;groupId?:string;purposeId?:string;campaignId?:string;amountMinor:number;currency:string;provider:string;idempotencyKey:string;anonymous?:boolean;note?:string;}
