...
async function platformBibleConfig(language="en"){
  ...
  const [youVersionKey,bibleBrainKey]=await Promise.all([
    optionalSecret("BIBLE_YOUVERSION_APP_KEY"),
    optionalSecret("BIBLE_BRAIN_API_KEY"),
  ]);
  return {
    providers:state.providers.map((item)=>({
      ...item,
      ready:item.provider_key==="getbible"
        ? true
        : item.provider_key==="youversion"
          ? Boolean(youVersionKey)
          : Boolean(bibleBrainKey),
