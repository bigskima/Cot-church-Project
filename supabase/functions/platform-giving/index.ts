...
function currency(value: unknown) {
  ...
  if (!/^[A-Z]{3}$/.test(result)) {
    throw new ApiError("VALIDATION_FAILED", "Currency must be a 3-letter ISO code", 422);
  }
  return result;
}
    ...
    async ({ request, requestId, auth }) => {
      ...
      if (request.method === "GET") {
        ...
        const organizationId = organizationIdParam
          ? uuid(organizationIdParam, "organizationId", true)!
          : null;
        ...
        for (const result of [organizations, settings, purposes, accounts]) {
          if (result.error) {
            throw new ApiError(
              "PLATFORM_GIVING_FAILED",
              "Unable to retrieve church-wide giving configuration",
              500,
              undefined,
              false,
            );
      ...
      if (action === "upsert_settings") {
        ...
        const organizationId = uuid(
          requiredString(body.organizationId, "organizationId", 64),
          "organizationId",
          true,
        )!;
        ...
        if (body.onlinePaymentEnabled === true) {
          throw new ApiError(
            ...
            "Online giving is not available until a real payment provider API and production adapter are installed and verified",
            409,
          );
        }
        ...
        const record = {
          ...
          online_unavailable_message:
            optionalString(body.onlineUnavailableMessage, "onlineUnavailableMessage", 500) ??
        ...
        if (result.error) {
          throw new ApiError(
            "GIVING_SETTINGS_SAVE_FAILED",
            "Unable to save church-wide giving settings",
            500,
            undefined,
            false,
          );
        ...
        if (auditError) {
          throw new ApiError("PLATFORM_AUDIT_FAILED", "Giving settings changed but audit recording failed", 500, undefined, false);
        }
      ...
      if (action === "upsert_purpose") {
        ...
        if (!purposeStatuses.has(status)) {
          throw new ApiError("VALIDATION_FAILED", "Invalid giving purpose status", 422);
        }
        const reason = requiredString(body.reason, "reason", 1000);
        ...
        if (isDefault) {
          ...
          if (error) {
            throw new ApiError(
              "GIVING_PURPOSE_SAVE_FAILED",
              "Unable to update default giving purpose",
              500,
              undefined,
              false,
            );
        ...
        if (result.error) {
          throw new ApiError(
            "GIVING_PURPOSE_SAVE_FAILED",
            "Unable to save church-wide giving purpose",
            500,
            undefined,
            false,
          );
        ...
        if (auditError) {
          throw new ApiError("PLATFORM_AUDIT_FAILED", "Giving purpose changed but audit recording failed", 500, undefined, false);
        }
        return { data: result.data };
      }
      ...
      if (action === "upsert_bank_account") {
        ...
        const record = {
          ...
          transfer_instructions:
            optionalString(body.transferInstructions, "transferInstructions", 2000) ?? "",
          additional_instructions:
            optionalString(body.additionalInstructions, "additionalInstructions", 2000) ?? "",
        ...
        if (result.error) {
          throw new ApiError(
            ...
            "Unable to save church-wide transfer account",
            500,
            undefined,
            false,
          );
        ...
        if (auditError) {
          throw new ApiError("PLATFORM_AUDIT_FAILED", "Transfer account changed but audit recording failed", 500, undefined, false);
        }
        return { data: result.data };
      }
