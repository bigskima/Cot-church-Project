...
function mediaUrl(value: unknown, field: string) {
...
Deno.serve(createHandler(
  ...
  async ({ request, auth }) => {
    ...
    const queryOrganizationId = requestedOrganizationId ? uuid(requestedOrganizationId, "organizationId", true)! : null;
    ...
    if (request.method === "GET") {
      ...
      if (!auth.organizationId) {
        ...
        if (authorizationError || authorized !== true) {
          throw new ApiError("EXPRESSION_CREATOR_AUTHORIZATION_REQUIRED", "Platform Authority has not authorized this account to create Expressions for this church", 403);
        }
        client = adminClient();
      }

      const { data, error } = await client.from("branches")
        .select("id, parent_branch_id, name, code, timezone, address, avatar_url, banner_url, is_active, created_at, updated_at")
    ...
    if (request.method === "POST") {
      ...
      if (!auth.organizationId) {
        const { data: authorized, error: authorizationError } = await auth.client.rpc("has_expression_creator_authorization", { target_organization_id: organizationId });
        if (authorizationError || authorized !== true) {
          throw new ApiError("EXPRESSION_CREATOR_AUTHORIZATION_REQUIRED", "Platform Authority has not authorized this account to create Expressions for this church", 403);
        }
      }
      const parentBranchId = body.parentBranchId === undefined || body.parentBranchId === null ? null : uuid(String(body.parentBranchId), "parentBranchId", true);
    ...
    if (!auth.organizationId) throw new ApiError("ORGANIZATION_REQUIRED", "Enter a church context before editing an Expression", 400);

    await authorize({ ...auth, branchId }, "branches.update");
    assertNoUnknownFields(body, ["name", "code", "timezone", "parentBranchId", "address", "avatarUrl", "bannerUrl", "isActive"]);
    ...
    if (body.avatarUrl !== undefined) updates.avatar_url = mediaUrl(body.avatarUrl, "avatarUrl");
    if (body.bannerUrl !== undefined) updates.banner_url = mediaUrl(body.bannerUrl, "bannerUrl");
    ...
    const { data, error } = await auth.client.from("branches").update(updates)
      ...
      .select("id, parent_branch_id, name, code, timezone, address, avatar_url, banner_url, is_active")
      .single();
    if (error?.code === "23505") throw new ApiError("EXPRESSION_CODE_TAKEN", "Expression code is already in use", 409);
