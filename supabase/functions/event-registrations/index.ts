...
Deno.serve(createHandler({ methods: ["GET", "POST", "DELETE"], authentication: "required", organization: "required" }, async ({ request, auth }) => {
  ...
  const { data: eventScope, error: eventScopeError } = await admin.from("events").select("id,organization_id,branch_id").eq("id", eventId).maybeSingle();
  if (eventScopeError || !eventScope) throw new ApiError("EVENT_NOT_FOUND", "Published event was not found", 404);
  await assertFeatureEnabled(admin, "events_gatherings", { organizationId: eventScope.organization_id, expressionId: eventScope.branch_id }, "Event registration is currently unavailable in this area.");
  if (request.method === "DELETE") { const { data, error } = await auth.client.rpc("cancel_event_registration", { target_event_id: eventId, target_occurrence_id: body.occurrenceId ? uuid(String(body.occurrenceId), "occurrenceId", true) : null }).single(); if (error?.code === "P0002") throw new ApiError("REGISTRATION_NOT_FOUND", "No active registration was found", 404); if (error) throw new ApiError("REGISTRATION_CANCEL_FAILED", "Unable to cancel registration", 500, undefined, false); return { data }; }
  const { data, error } = await auth.client.rpc("register_for_event", { target_event_id: eventId, target_occurrence_id: body.occurrenceId ? uuid(String(body.occurrenceId), "occurrenceId", true) : null }).single();
