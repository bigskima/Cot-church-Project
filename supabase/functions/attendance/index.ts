...
Deno.serve(createHandler({ methods: ["GET", "POST"], authentication: "required", organization: "required" }, async ({ request, auth }) => {
  ...
  const { data: eventScope } = await admin.from("events").select("organization_id,branch_id").eq("id", eventId).maybeSingle();
  await assertFeatureEnabled(
    admin,
    "attendance",
    { organizationId: eventScope?.organization_id ?? auth.organizationId, expressionId: eventScope?.branch_id ?? auth.branchId },
    "Attendance check-in is currently unavailable in this area.",
  );
  const { data, error } = await auth.client.rpc("check_in_member", { target_event_id: eventId, target_occurrence_id: body.occurrenceId ? uuid(String(body.occurrenceId), "occurrenceId", true) : null, target_membership_id: uuid(requiredString(body.membershipId, "membershipId", 36), "membershipId", true), check_in_notes: optionalString(body.notes, "notes", 500) ?? "" }).single();
