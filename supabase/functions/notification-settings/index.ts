    ...
    async ({ request, auth }) => {
      ...
      if (request.method === "POST") {
        ...
        const { data, error } = await auth.client
          ...
          .upsert(
            {
              ...
              expo_push_token: requiredString(body.expoPushToken, "expoPushToken", 255),
              platform,
          ...
          .select()
          .single();
        if (error) throw new ApiError("DEVICE_REGISTER_FAILED", "Unable to register device", 500, undefined, false);
      ...
      if (request.method === "DELETE") {
        ...
        const { error } = await auth.client
          ...
          .eq("profile_id", auth.user.id)
          .eq("expo_push_token", requiredString(body.expoPushToken, "expoPushToken", 255));
        if (error) throw new ApiError("DEVICE_REMOVE_FAILED", "Unable to remove device", 500, undefined, false);
        return { data: { status: "deactivated" } };
      }

      assertNoUnknownFields(body, ["emailEnabled", "smsEnabled", "pushEnabled", "quietHours"]);
