...
Deno.serve(createHandler({ methods: ["GET", "POST", "PATCH"], authentication: "required", organization: "required" }, async ({ request, auth }) => {
  ...
  await authorize(auth, "events.update"); assertNoUnknownFields(body, ["id", "title", "description", "status", "visibility", "location", "timezone", "startsAt", "endsAt", "capacity"]);
  ...
  if (body.location !== undefined) { if (!body.location || typeof body.location !== "object" || Array.isArray(body.location)) throw new ApiError("VALIDATION_FAILED", "Location must be an object", 422); updates.location = body.location; }
