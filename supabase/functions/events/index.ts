...
function timestamp(value: unknown, field: string) {
  ...
  if (Number.isNaN(Date.parse(result))) throw new ApiError("VALIDATION_FAILED", `${field} must be an ISO timestamp`, 422);
  return result;
}
...
Deno.serve(createHandler(
  ...
  async ({ request, auth }) => {
    ...
    if (request.method === "GET") {
      ...
      else query = query.gte("ends_at", url.searchParams.get("from") ?? new Date().toISOString()).limit(100);
      const { data, error } = await query;
      if (error) throw new ApiError("EVENT_LIST_FAILED", "Unable to retrieve events", 500, undefined, false);
    ...
    if (request.method === "POST" && body.action === "create_banner_upload") {
      ...
      if (error || !data?.signedUrl) throw new ApiError("UPLOAD_SESSION_FAILED", "Unable to prepare event banner upload", 500, undefined, false);
    ...
    if (request.method === "POST") {
      ...
      if (capacity !== null && (!Number.isInteger(capacity) || capacity < 1)) throw new ApiError("VALIDATION_FAILED", "capacity must be a positive integer", 422);
      ...
      const record = {
        ...
        branch_id: requestedBranchId ?? auth.branchId ?? null,
        ...
        starts_at: timestamp(body.startsAt, "startsAt"),
        ends_at: timestamp(body.endsAt, "endsAt"),
        registration_opens_at: body.registrationOpensAt ? timestamp(body.registrationOpensAt, "registrationOpensAt") : null,
        registration_closes_at: body.registrationClosesAt ? timestamp(body.registrationClosesAt, "registrationClosesAt") : null,
        capacity,
      ...
      if (error) throw new ApiError("EVENT_CREATE_FAILED", "Unable to create event", 500, undefined, false);
    ...
    if (body.location !== undefined) {
      if (!body.location || typeof body.location !== "object" || Array.isArray(body.location)) throw new ApiError("VALIDATION_FAILED", "Location must be an object", 422);
      updates.location = body.location;
    }
    if (body.timezone !== undefined) updates.timezone = requiredString(body.timezone, "timezone", 64);
    if (body.startsAt !== undefined) updates.starts_at = timestamp(body.startsAt, "startsAt");
    if (body.endsAt !== undefined) updates.ends_at = timestamp(body.endsAt, "endsAt");
    if (body.capacity !== undefined) {
      const capacity = body.capacity === null ? null : Number(body.capacity);
      if (capacity !== null && (!Number.isInteger(capacity) || capacity < 1)) throw new ApiError("VALIDATION_FAILED", "Invalid capacity", 422);
    ...
    if (error?.code === "PGRST116") throw new ApiError("EVENT_NOT_FOUND", "This event is not available in the active context", 404);
    if (error) throw new ApiError("EVENT_UPDATE_FAILED", "Unable to update event", 500, undefined, false);
    return { data };
  },
));
