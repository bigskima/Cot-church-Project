...
Deno.serve(createHandler(
  ...
  async ({ request, auth }) => {
    ...
    let sermonQuery = client
      ...
      .eq("id", sermonId);
    if (auth?.organizationId) sermonQuery = sermonQuery.eq("organization_id", auth.organizationId);

    const { data: sermon, error: sermonError } = await sermonQuery.maybeSingle();
    ...
    const { data: recording, error: recordingError } = await admin
      ...
      .select("id,organization_id,stream_id,playback_id,status,duration_seconds,audio_only_url,download_url,live_streams!inner(id,organization_id,branch_id,provider_config_id,provider_metadata,thumbnail_url)")
      .eq("id", sermon.recording_id)
      .eq("organization_id", sermon.organization_id)
      .maybeSingle();
    if (recordingError || !recording) throw new ApiError("SERMON_RECORDING_NOT_FOUND", "The sermon recording is unavailable", 404);
    if (recording.status !== "ready") throw new ApiError("SERMON_RECORDING_PROCESSING", "The sermon recording is still processing", 409);
    ...
    const grant = await streamingProvider(loaded.provider.providerCode).createPlaybackToken(loaded.provider, playbackId, 600);
    const protectedAudioUrl = sermon.visibility === "public" ? recording.audio_only_url : null;
    ...
    return {
      data: {
        ...
        videoUrl: grant.url,
        audioUrl: protectedAudioUrl ?? grant.url,
