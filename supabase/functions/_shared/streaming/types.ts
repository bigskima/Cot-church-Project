...
export type StreamingRouteScope='general'|'expression';
...
export interface ProvisionedBroadcast{
 providerBroadcastId:string;
 ingest:{protocols:Array<'rtmp'|'srt'|'webrtc'>;rtmpUrl?:string;srtUrl?:string;streamKey?:string};
 playbackId?:string;
 publicPlaybackUrl?:string;
 rtc?:{channelName:string};
 raw:Record<string,unknown>;
}
export interface PlaybackGrant{url:string;token?:string;expiresAt:string}
export interface RtcGrant{provider:'agora';appId:string;channelName:string;token:string;uid:number;role:'publisher'|'subscriber';expiresAt:string}
...
export interface StreamingProvider{
 ...
 createBroadcast(config:ProviderConfiguration,request:BroadcastRequest):Promise<ProvisionedBroadcast>;
 createIngestEndpoint(config:ProviderConfiguration,broadcastId:string):Promise<ProvisionedBroadcast['ingest']>;
 getStreamStatus(config:ProviderConfiguration,broadcastId:string):Promise<StreamLifecycle>;
 startBroadcast(config:ProviderConfiguration,broadcastId:string):Promise<void>;
 stopBroadcast(config:ProviderConfiguration,broadcastId:string):Promise<void>;
 ...
 createRtcGrant?(config:ProviderConfiguration,channelName:string,uid:number,role:'publisher'|'subscriber',ttlSeconds:number):Promise<RtcGrant>;
 verifyWebhook(config:ProviderConfiguration,rawBody:string,headers:Headers):Promise<ProviderWebhook>;
 getRecording(config:ProviderConfiguration,assetId:string):Promise<Record<string,unknown>>;
 createClip(config:ProviderConfiguration,assetId:string,startSeconds:number,endSeconds:number):Promise<{providerClipId:string}>;
