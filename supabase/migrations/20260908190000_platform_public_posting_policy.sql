...
revoke all on function public.publish_social_post(uuid, public.content_visibility, text, uuid, uuid, jsonb) from public, anon;
grant execute on function public.publish_social_post(uuid, public.content_visibility, text, uuid, uuid, jsonb) to authenticated, service_role;
...
begin
  ...
  if p_expression_id is null
     and p_group_id is null
     and p_visibility = 'public'::public.content_visibility
     and not public.can_profile_post_publicly(auth.uid()) then
    raise exception using errcode='42501', message='Public posting is currently unavailable for this account';
  end if;
