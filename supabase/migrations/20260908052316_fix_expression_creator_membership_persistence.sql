...
begin
  ...
  if parent_expression_id is not null and not exists (
    ...
    where id=parent_expression_id and organization_id=target_organization_id and is_active
  ) then
  ...
  values(target_organization_id,parent_expression_id,trim(expression_name),upper(trim(expression_code)),
         coalesce(nullif(trim(expression_timezone),''),'UTC'),expression_address)
  returning * into created_branch;

  insert into public.memberships(organization_id,branch_id,profile_id,status,joined_at)
  values(target_organization_id,null,auth.uid(),'active',current_date)
  on conflict(organization_id,profile_id)
  do update set status='active',joined_at=coalesce(public.memberships.joined_at,current_date)
  ...
  values(target_organization_id,created_branch.id,membership.id,auth.uid(),'active',now(),null)
  ...
  values(target_organization_id,created_branch.id,auth.uid(),'create','expression',created_branch.id::text,
         jsonb_build_object('name',created_branch.name,'code',created_branch.code,'ownerProfileId',auth.uid()));
